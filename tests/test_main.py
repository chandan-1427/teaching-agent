...
from ai_teaching_agent.main import CourseState, handler, initialize_agent, load_skill, parallel_team_node
...
async def test_handler_success_flow(mock_messages):
    ...
    with patch("ai_teaching_agent.main._initialized", True), patch("ai_teaching_agent.main.graph") as mock_graph:
...
async def test_parallel_team_exception_handling(mock_llm_response):
    ...
    side_effects = [mock_llm_response, Exception("LLM Timeout Error"), mock_llm_response]

    with (
        patch("ai_teaching_agent.main.global_llm") as mock_llm,
...
async def test_load_skill_regex_resilience():
    ...
    with patch("pathlib.Path.read_text", return_value=mock_content), patch("pathlib.Path.exists", return_value=True):
...
async def test_initialization_lock_safety():
    ...
    with (
        patch("ai_teaching_agent.main._initialized", False),
        ...
        await asyncio.gather(
            handler([{"role": "user", "content": "Topic A"}]), handler([{"role": "user", "content": "Topic B"}])
        )
...
async def test_missing_api_key_runtime_error():
    ...
    with (
        ...
        pytest.raises(RuntimeError, match="Missing OPENROUTER_API_KEY"),
    ):
...
async def test_handler_llm_timeout_resilience():
    ...
    with patch("ai_teaching_agent.main._initialized", True), patch("ai_teaching_agent.main.graph") as mock_graph:
        ...
        mock_graph.ainvoke = AsyncMock(side_effect=TimeoutError("Request timed out"))
