...
run: env-check ## Run the agent locally
	@echo "🤖 Starting ai-teaching-agent..."
	@uv run python -m ai_teaching_agent

.PHONY: run-dev
run-dev: ## Run the agent in development mode with auto-reload
	@echo "🤖 Starting ai-teaching-agent in development mode..."
	@uv run watchfiles "python -m ai_teaching_agent" ai_teaching_agent/
...
docker-build-no-cache: ## Build Docker image without cache
	...
	@docker build --no-cache -f Dockerfile.agent -t ai_teaching_agent:latest .

.PHONY: docker-run
docker-run: ## Run Docker container with .env file
	@echo "🐳 Running Docker container..."
	@docker run -d \
		--name ai_teaching_agent \
		-p 3773:3773 \
		--env-file .env \
		--restart unless-stopped \
		ai_teaching_agent:latest
...
docker-clean: docker-rm ## Clean up Docker images and containers
	...
	@docker rmi ai_teaching_agent:latest || true
...
docker-push: ## Push Docker image to Docker Hub
	...
	@docker tag ai_teaching_agent:latest chandan/ai_teaching_agent:latest
	@docker push chandan/ai_teaching_agent:latest
...
status: ## Show agent and Docker status
	...
	@docker ps -a | grep ai_teaching_agent || echo "No containers found"
	@echo "\nDocker Images:"
	@docker images | grep ai_teaching_agent || echo "No images found"
	@echo "\nAgent Health:"
